...
Гостевой чат на сайте использует **тот же** workflow (второй триггер — Webhook) — см. [N8N_GUEST_CHAT_SETUP_RU.md](N8N_GUEST_CHAT_SETUP_RU.md). Отдельный workflow и повторная настройка секретов не нужны. n8n с телефона для ответов гостям не нужен.
